...
namespace _365Beauty.Query.Application.UserCases.BeautySalons.BeautySalonServices
{
    public class GetDetailBeautySalonServiceHandler : IRequestHandler<GetDetailBeautySalonServiceQuery, Result<BeautySalonServiceFullDTO>>
    {
        ...
        {
            var salonService = beautySalonServiceRepository.FindAll(false, x=> x.Id == request.Id, x => x.Price!, x => x.SalonCatalog!, x => x.ServiceCatalog!).Where(x => x.Price != null && x.Price.IsActived == StatusActived.Actived).FirstOrDefault();
            ...
            var entity = new BeautySalonServiceFullDTO
            {
                ...
                PrecentDiscount = (salonService.Price != null && salonService.Price.BasePrice > 0) ? (int)Math.Round((salonService.Price.BasePrice - salonService.Price.FinalPrice) / salonService.Price.BasePrice * 100) : 0,
