using _365Beauty.Contract.Helpers;
...
namespace Commerce.Command.Contract.Helpers
{
    public class Vnpay : IVnpay
    {
        ...
        public VnpayResult ProcessCallback(HttpRequest request)
        {
            var query = request.Query;
            string vnp_SecureHash = query["vnp_SecureHash"];
            string paymentId = query["vnp_TxnRef"];
            ...
            string computedHash = CreateSha256(_hashSecret + rawData);

            if (computedHash == vnp_SecureHash && responseCode == "00")
            {
